...
def handleErrorMessage(client):
    message='''
    you need to connect with atleast with one client to send message and click on refresh to see all the available users'''
    client.send(message.encode())

def sendTextMessage(client_name,message):
    global clients
    other_clientName=clients[client_name]["connected_with"]
    other_client_socket = clients[other_clientName]["client"]
...
def disconnectWithClient(message, client, client_name):
    ...
    if(entered_client_name in clients):
        ...
        greet_message = f"Hello, {entered_client_name} disconnected with {client_name}!!!"
        other_client_socket.send(greet_message.encode())

        msg = f"You are successfully disconnected with {entered_client_name}"
        client.send(msg.encode())
        # else:
        #     other_client_name = clients[client_name]["disconnected_with"]
        #     msg = f"You are already disconnected with {other_client_name}"
        #     client.send(msg.encode())

def grandAccess(client_name):
    global clients
...
def handleMessges(client, message, client_name):
    if(message == 'show list'):
        ...
        grandAccess(client_name)
    elif(message=="n" or message=="no"):
        declineAccess(client_name)
    else:
        connected=client[client_name]["connected_with"]
        if connected==True:
